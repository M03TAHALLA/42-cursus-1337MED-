...
static char	*join_buffer(char *result, char *buffer)
{
	...
	new_buffer = ft_strjoinget(result, buffer);
	free(result);
	return (new_buffer);
}
...
static char	*extract_line_from_buffer(char **result)
{
	...
	len = ft_strchrget(*result, '\n') - *result + 1;
	line = ft_strndupget(*result, len);
	new_buffer = ft_strdupget(*result + len);
	free(*result);
	*result = new_buffer;
...
char	*get_next_line(int fd)
{
	...
	if (!result || result[0] == '\0')
		return (free(result), result = NULL, NULL);
	if (ft_strchrget(result, '\n'))
		line = extract_line_from_buffer(&result);
	else
	{
		line = ft_strdupget(result);
		free(result);
		result = NULL;
	}
	return (line);
